...
def delete_account(username):
    ...
    if user_data:
        ...
        # --- Remove PDF records from cloud (Google Drive) ---
        # PDFs are now stored in Google Drive; optionally remove them from Drive if desired
        ...
        for pdf in pdf_history:
            drive_id = pdf.get("drive_id")
            if drive_id:
                try:
                    drive_service.files().delete(fileId=drive_id).execute()
                except Exception as e:
                    st.warning(f"⚠️ Could not delete PDF from Drive (ID: {drive_id}): {e}")
